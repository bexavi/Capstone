...
export function useFileUploadWithStatus(chatId: MaybeRefOrGetter<string>) {
  ...
  const uploadPath = computed(() => `/api/upload/${toValue(chatId)}`)
  const upload = useUpload(uploadPath as unknown as string, {
    method: 'PUT',
